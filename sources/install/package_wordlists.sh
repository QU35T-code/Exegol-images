...
function install_seclists() {
    ...
    git -C /opt/tools clone --single-branch --branch master --depth 1 https://github.com/danielmiessler/SecLists.git seclists
    cd /opt/tools/seclists
    rm -r LICENSE .git* CONTRIBUT* .bin
    add-test-command "[ -d '/opt/tools/seclists/Discovery/' ]"
...
function configure_seclists() {
    ...
    ln -v -s /opt/tools/seclists /usr/share/seclists
    ln -v -s /opt/tools/seclists /usr/share/wordlists/seclists
}

function configure_rockyou() {
    colorecho "Configuring rockyou"
    tar -xvf /opt/tools/seclists/Passwords/Leaked-Databases/rockyou.txt.tar.gz -C /opt/
